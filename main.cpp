...
//	}
	...
	unsigned long size = maximal_cliques.size();

	auto num_combos = (long long) pow(2, size);
	...
	for (int i = 1; i < num_combos; i++) {
		...
		for (int j = 0; j < size; j++) {
			// creates every possible combination of the maximal cliques
		...
		// save some time by only printing progress every thousand or so iterations
		if (i % 2048 == 0) {
			std::cout << "Percent Complete: " << ((float) i / num_combos) * 100 << "%            \r"; // these spaces are necessary
        }
        ...
        if (result.empty() || candidate.size() < result.size()) {
            std::cout << "found new minimal clique cover, size = " << candidate.size() << std::endl;
            result = candidate;
        }
	}
	std::cout << std::endl;
